...
create table if not exists public.season_flight_records (
  ...
  turnaround_id text,
  primary key (season_id, record_id)
);
...
create table if not exists public.season_flight_record_counters (
  ...
  primary key (season_id, record_id, counter_group, item_index),
  foreign key (season_id, record_id) references public.season_flight_records(season_id, record_id) on delete cascade
);
...
create table if not exists public.season_flight_record_checkin_windows (
  ...
  primary key (season_id, record_id, counter_key),
  foreign key (season_id, record_id) references public.season_flight_records(season_id, record_id) on delete cascade
);
...
create table if not exists public.season_modification_counters (
  ...
  item_index integer not null default 0,
  counter_value text not null,
  primary key (season_id, leg_id, counter_group, item_index),
  foreign key (season_id, leg_id) references public.season_modifications(season_id, leg_id) on delete cascade
);
...
create table if not exists public.season_modification_checkin_windows (
  ...
  window_start text not null,
  window_end text not null,
  primary key (season_id, leg_id, counter_key),
  foreign key (season_id, leg_id) references public.season_modifications(season_id, leg_id) on delete cascade
);
...
create table if not exists public.season_mod_history_changes (
  ...
  primary key (season_id, entry_id, change_index),
  foreign key (season_id, entry_id) references public.season_mod_history_entries(season_id, entry_id) on delete cascade
);
...
create table if not exists public.season_change_events (
  ...
  created_at timestamptz not null default now(),
  unique (season_id, client_id, op_id)
);
...
create index if not exists season_flight_records_date_idx on public.season_flight_records (season_id, date, flight_number);
create index if not exists season_flight_records_reporting_idx on public.season_flight_records (season_id, status, type, airline, route, aircraft, date);
...
create index if not exists season_change_events_target_idx on public.season_change_events (season_id, target_type, target_id);
...
begin
  insert into public.season_flight_records (
    ...
    mct, fb, lb, bhs, ghs, date, day_of_week, action, source_row_index, linked_source_row_index,
  ...
  values (
    ...
    record_payload->>'bhs', record_payload->>'ghs', coalesce(record_payload->>'date', ''), coalesce(nullif(record_payload->>'dayOfWeek', '')::integer, 0),
  ...
  on conflict (season_id, record_id) do update set
  ...
  delete from public.season_flight_record_counters where season_id = p_season_id and record_id = v_record_id;
  ...
  if v_counter is not null and jsonb_typeof(v_counter) <> 'null' then
    if jsonb_typeof(v_counter) = 'array' then
      ...
      loop
        insert into public.season_flight_record_counters values (p_season_id, v_record_id, '__single__', v_index, trim(both '"' from v_item::text));
        v_index := v_index + 1;
      end loop;
      ...
      loop
        if jsonb_typeof(v_counter_value) = 'array' then
          ...
          loop
            insert into public.season_flight_record_counters values (p_season_id, v_record_id, v_counter_key, v_index, trim(both '"' from v_item::text));
            v_index := v_index + 1;
          end loop;
        else
          insert into public.season_flight_record_counters values (p_season_id, v_record_id, v_counter_key, 0, trim(both '"' from v_counter_value::text));
        end if;
      end loop;
    else
      insert into public.season_flight_record_counters values (p_season_id, v_record_id, '__single__', 0, trim(both '"' from v_counter::text));
    end if;
  end if;

  delete from public.season_flight_record_checkin_windows where season_id = p_season_id and record_id = v_record_id;
  for v_window in select key, value from jsonb_each(coalesce(record_payload->'checkInCounterWindows', '{}'::jsonb))
  loop
    insert into public.season_flight_record_checkin_windows (season_id, record_id, counter_key, window_start, window_end)
    values (p_season_id, v_record_id, v_window.key, v_window.value->>'start', v_window.value->>'end')
    on conflict (season_id, record_id, counter_key) do update set window_start = excluded.window_start, window_end = excluded.window_end;
  end loop;
end;
$$;
...
begin
  ...
  on conflict (season_id, leg_id) do update set
    action = excluded.action,
  ...
  delete from public.season_modification_counters where season_id = p_season_id and leg_id = v_leg_id;
  ...
  if v_counter is not null and jsonb_typeof(v_counter) <> 'null' then
    if jsonb_typeof(v_counter) = 'array' then
      ...
      for v_item in select * from jsonb_array_elements(v_counter)
      loop
        insert into public.season_modification_counters values (p_season_id, v_leg_id, '__single__', v_index, trim(both '"' from v_item::text));
        v_index := v_index + 1;
      end loop;
    elsif jsonb_typeof(v_counter) = 'object' then
      for v_counter_key, v_counter_value in select * from jsonb_each(v_counter)
      loop
        if jsonb_typeof(v_counter_value) = 'array' then
          ...
          for v_item in select * from jsonb_array_elements(v_counter_value)
          loop
            insert into public.season_modification_counters values (p_season_id, v_leg_id, v_counter_key, v_index, trim(both '"' from v_item::text));
            v_index := v_index + 1;
          end loop;
        else
          insert into public.season_modification_counters values (p_season_id, v_leg_id, v_counter_key, 0, trim(both '"' from v_counter_value::text));
        end if;
      end loop;
    else
      insert into public.season_modification_counters values (p_season_id, v_leg_id, '__single__', 0, trim(both '"' from v_counter::text));
    end if;
  end if;

  delete from public.season_modification_checkin_windows where season_id = p_season_id and leg_id = v_leg_id;
  for v_window in select key, value from jsonb_each(coalesce(mod_payload->'checkInCounterWindows', '{}'::jsonb))
  loop
    insert into public.season_modification_checkin_windows (season_id, leg_id, counter_key, window_start, window_end)
    values (p_season_id, v_leg_id, v_window.key, v_window.value->>'start', v_window.value->>'end')
    on conflict (season_id, leg_id, counter_key) do update set window_start = excluded.window_start, window_end = excluded.window_end;
  end loop;

  delete from public.season_modification_added_legs where season_id = p_season_id and leg_id = v_leg_id;
  ...
  if added_leg is not null and jsonb_typeof(added_leg) = 'object' then
    ...
    values (
      ...
      added_leg->>'bhs', added_leg->>'ghs', coalesce(added_leg->>'date', ''), coalesce(nullif(added_leg->>'dayOfWeek', '')::integer, 0),
...
begin
  ...
  on conflict (season_id, entry_id) do update set timestamp = excluded.timestamp, description = excluded.description;
  delete from public.season_mod_history_changes where season_id = p_season_id and entry_id = v_entry_id;
  delete from public.season_mod_history_record_changes where season_id = p_season_id and entry_id = v_entry_id;
  for v_change in select * from jsonb_array_elements(coalesce(history_payload->'changes', '[]'::jsonb))
  loop
    insert into public.season_mod_history_changes (season_id, entry_id, change_index, leg_id, previous_mod_snapshot, new_mod_snapshot)
    values (p_season_id, v_entry_id, v_index, v_change->>'legId', v_change->'previousMod', coalesce(v_change->'newMod', '{}'::jsonb));
    v_index := v_index + 1;
  end loop;
  v_index := 0;
  for v_change in select * from jsonb_array_elements(coalesce(history_payload->'recordChanges', '[]'::jsonb))
  loop
    insert into public.season_mod_history_record_changes (season_id, entry_id, change_index, record_id, previous_record_snapshot, new_record_snapshot)
    values (p_season_id, v_entry_id, v_index, v_change->>'recordId', v_change->'previousRecord', v_change->'newRecord');
    v_index := v_index + 1;
  end loop;
end;
$$;
...
begin
  if op_type = 'sourceRow' then
    ...
    delete from public.season_modifications where season_id = p_season_id and leg_id = op->>'legId';
    ...
    if has_conflict then
      ...
      loop
        next_field_versions := jsonb_set(
          next_field_versions,
          array[changed_field],
          to_jsonb(coalesce((next_field_versions->>changed_field)::bigint, 0) + 1),
          true
        );
      end loop;
      ...
      on conflict (season_id, client_id, op_id) do update set op_payload = excluded.op_payload
...
select
  ...
  r.pax,
  r.date as ops_date,
  to_char(to_date(nullif(r.date, ''), 'YYYY-MM-DD'), 'YYYY-MM') as month,
  extract(week from to_date(nullif(r.date, ''), 'YYYY-MM-DD'))::integer as iso_week,
  nullif(split_part(r.schedule, ':', 1), '')::integer as local_hour,
  ((nullif(split_part(r.schedule, ':', 1), '')::integer + 17) % 24) as utc_hour,
  r.day_of_week as weekday,
  r.status,
  r.gate,
  r.stand,
  r.carousel,
