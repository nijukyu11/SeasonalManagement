...
begin
  insert into public.season_source_rows (
    season_id, row_index, effective, discontinue, airline, aircraft, sta, arr_flight, arr_flight_type, arr_route,
    arr_category, arr_code_shares, arr_int_dom_ind, std, dep_flight, dep_flight_type, dep_route, dep_category,
  ...
  values (
    ...
    row_payload->>'arrFlight', row_payload->>'arrFlightType', row_payload->>'arrRoute', row_payload->>'arrFlightCategory',
    row_payload->>'arrCodeShares', row_payload->>'arrIntDomInd', row_payload->>'std',
    row_payload->>'depFlight', row_payload->>'depFlightType', row_payload->>'depRoute', row_payload->>'depFlightCategory',
...
begin
  ...
  if v_batch.apply_strategy <> 'merge' then
    raise exception 'Seasonal import batch % is not a merge preview', p_batch_id
      using errcode = '22023';
  end if;
  ...
  perform 1
  from public.season_flight_records records
  join pg_temp.seasonal_import_commit_records_v3 incoming
    on incoming.final_record_id = records.record_id
  ...
  select
    ...
    coalesce(
      (
        ...
        from public.season_modification_counters counters
        where counters.leg_id = incoming.final_record_id
      ),
      '[]'::jsonb
    ),
    coalesce(
      (
        ...
        from public.season_modification_checkin_windows windows
        where windows.leg_id = incoming.final_record_id
      ),
      '[]'::jsonb
    ),
    (
      ...
      from public.season_modification_added_legs added_legs
      where added_legs.season_id = v_target_season_id
        and added_legs.leg_id = incoming.final_record_id
    )
  from pg_temp.seasonal_import_commit_records_v3 incoming
  left join public.season_flight_records records
    on records.record_id = incoming.final_record_id;
  ...
  insert into public.season_change_events (
    ...
    'seasonImport',
    v_target_season_id,
    array['flightRecords', 'seasonMetadata'],
    pg_catalog.jsonb_build_object(
...
-- Keep the canonical clean-schema result aligned with the final RLS InitPlan migration.
-- This must remain after every policy-creation block above.
do $$
declare
  table_name text;
...
-- Workspace window V2 canonical schema mirror.
...
-- Seasonal RLS permission InitPlan optimization canonical schema mirror.
