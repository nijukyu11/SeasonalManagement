...
export interface LocalSeasonSqlPendingSummary {
  seasonId: string;
  pendingCount: number;
...
const SCHEMA_STATEMENTS = [
  ...
  'CREATE INDEX IF NOT EXISTS idx_local_flight_records_operational_lookup ON local_flight_records (season_id, is_base, operational_date, type, status)',
...
const SQL_UNAVAILABLE_MESSAGE = 'SQL local season store is unavailable';

let databasePromise: Promise<LocalSeasonSqlDatabase> | null = null;
let databaseFactory: (() => Promise<LocalSeasonSqlDatabase | null>) | null = null;
let sqlWriteQueueTail: Promise<unknown> = Promise.resolve();
...
function isNestedTransactionError(error: unknown): boolean {
  if (typeof error === 'string') return /cannot start a transaction within a transaction/i.test(error);
  if (error instanceof Error) return isNestedTransactionError(error.message);
  if (error && typeof error === 'object') {
    const record = error as { code?: unknown; message?: unknown };
    return isNestedTransactionError([record.code, record.message].filter(Boolean).join(' '));
  }
  return false;
}
...
async function enqueueSqlWrite<T>(label: string, write: () => Promise<T>): Promise<T> {
  const queuedAt = Date.now();
  const previous = sqlWriteQueueTail.catch(() => undefined);
  const current = previous.then(async () => {
    const waitedMs = Date.now() - queuedAt;
    if (waitedMs > SQL_WRITE_QUEUE_WARN_MS) {
...
async function beginTransactionWithRecovery(
  db: LocalSeasonSqlDatabase,
  label: string
): Promise<void> {
  try {
    ...
    console.warn(`[local-season-sql] recovering nested transaction before ${label}`);
    try {
      await executeSql(db, 'ROLLBACK');
    } catch (rollbackError) {
      console.warn('[local-season-sql] rollback during nested transaction recovery failed', rollbackError);
      throw error;
    }
    try {
      await executeSql(db, 'BEGIN IMMEDIATE');
...
function sqlUnavailableError(detail?: unknown): Error {
  if (!detail) return new Error(SQL_UNAVAILABLE_MESSAGE);
  const message = detail instanceof Error ? detail.message : String(detail);
  return new Error(`${SQL_UNAVAILABLE_MESSAGE}: ${message}`);
}

export async function getLocalSeasonSqlDatabase(): Promise<LocalSeasonSqlDatabase> {
  if (!databasePromise) {
    databasePromise = (databaseFactory ?? loadTauriSqlDatabase)()
      .then((db) => {
        if (!db) throw sqlUnavailableError();
        return db;
      })
      .catch((error) => {
        throw error instanceof Error && error.message.startsWith(SQL_UNAVAILABLE_MESSAGE)
          ? error
          : sqlUnavailableError(error);
      });
  }
...
async function loadTauriSqlDatabase(): Promise<LocalSeasonSqlDatabase | null> {
  if (!isTauriRuntime()) throw sqlUnavailableError('Tauri SQL runtime is not available');
  try {
    ...
    throw sqlUnavailableError(error);
  }
}
...
export async function saveLocalSeasonSqlWorkspace(
  ...
  const pendingOps = activePendingOps(workspace.pendingOps);
  const syncMeta = normalizeSyncMetaForPendingOps(workspace.syncMeta, pendingOps);
  await transaction(db, options.writeLabel ?? 'workspace-save', async () => {
    await executeSql(db, 
...
function activePendingOps(pendingOps: LocalPendingOp[]): LocalPendingOp[] {
  return pendingOps.filter((op) => op.type !== 'sourceRow');
}

function normalizeSyncMetaForPendingOps(syncMeta: LocalSyncMeta, pendingOps: LocalPendingOp[]): LocalSyncMeta {
  const pendingCount = pendingOps.length;
  const conflictCount = syncMeta.conflicts?.length ?? 0;
  const syncStatus = pendingCount > 0
    ? (syncMeta.syncStatus === 'synced' ? 'dirty' : syncMeta.syncStatus)
    : (conflictCount > 0 ? 'needs_review' : 'synced');
  return {
    ...syncMeta,
    pendingCount,
    syncStatus,
    lastLocalChangeAt: pendingCount > 0 ? syncMeta.lastLocalChangeAt : null,
  };
}
...
async function loadPendingOps(
  ...
  return activePendingOps(rows.map((row) => payload<LocalPendingOp>(row, null as unknown as LocalPendingOp)));
}
...
export async function clearLocalSeasonSqlWorkspaces(db: LocalSeasonSqlDatabase): Promise<void> {
  await transaction(db, 'clear-workspaces', async () => {
    for (const table of [
      'local_source_rows',
      'local_flight_records',
      'local_modifications',
...
export async function listLocalSeasonSqlPendingSummaries(
  db: LocalSeasonSqlDatabase
): Promise<LocalSeasonSqlPendingSummary[]> {
  const rows = await db.select<JsonRow & {
    seasonId?: string;
    season_id?: string;
    pendingCount?: number;
    ...
    syncStatus?: LocalSyncMeta['syncStatus'];
    sync_status?: LocalSyncMeta['syncStatus'];
    lastLocalChangeAt?: number | null;
    ...
    `SELECT
     ...
     FROM local_sync_meta
     ORDER BY season_id ASC`
  );
  return rows.map((row) => {
    const seasonId = String(row.seasonId ?? row.season_id ?? '');
    const meta = payload<Partial<LocalSyncMeta>>(row, {});
    const pendingCount = Number(row.pendingCount ?? row.pending_count ?? meta.pendingCount ?? 0);
    const conflictCount = Array.isArray(meta.conflicts) ? meta.conflicts.length : 0;
    const syncStatus = conflictCount > 0 && pendingCount === 0
      ? 'needs_review'
      : (row.syncStatus ?? row.sync_status ?? meta.syncStatus ?? (pendingCount > 0 ? 'dirty' : 'synced'));
    return {
      seasonId,
      pendingCount,
      conflictCount,
      syncStatus,
      lastLocalChangeAt: row.lastLocalChangeAt ?? row.last_local_change_at ?? meta.lastLocalChangeAt ?? null,
    };
  });
}

export async function discardLocalSeasonSqlPendingChanges(
  db: LocalSeasonSqlDatabase,
  seasonId: string
): Promise<{ discardedCount: number; syncMeta: LocalSyncMeta } | null> {
  const syncMeta = await readLocalSeasonSqlSyncMeta(db, seasonId);
  if (!syncMeta) return null;
  const pendingRows = await db.select<{ count?: number }>(
    'SELECT COUNT(*) AS count FROM local_pending_ops WHERE season_id = ?',
    [seasonId]
  );
  const discardedCount = Math.max(syncMeta.pendingCount ?? 0, Number(pendingRows[0]?.count ?? 0));
  const conflictCount = syncMeta.conflicts?.length ?? 0;
  const nextSyncMeta: LocalSyncMeta = {
    ...syncMeta,
    localRevision: (syncMeta.localRevision ?? 0) + 1,
    pendingCount: 0,
    lastLocalChangeAt: null,
    syncStatus: conflictCount > 0 ? 'needs_review' : 'synced',
  };

  await transaction(db, 'session-discard', async () => {
    await executeSql(db, 'DELETE FROM local_source_rows WHERE season_id = ?', [seasonId]);
    await executeSql(db, 'DELETE FROM local_flight_records WHERE season_id = ? AND is_base = 0', [seasonId]);
    await executeSql(db,
      `INSERT INTO local_flight_records (
        season_id, record_id, is_base, sort_order, flight_date, operational_date,
        type, source_side, status, turnaround_id, gate, stand, counter_json,
    ...
    await executeSql(db, 'DELETE FROM local_modifications WHERE season_id = ? AND is_base = 0', [seasonId]);
    await executeSql(db,
      `INSERT INTO local_modifications (
        season_id, leg_id, is_base, sort_order, action, gate, stand, counter_json,
        check_in_start, check_in_end, schedule, payload_json
    ...
    await executeSql(db, 'DELETE FROM local_mod_history_entries WHERE season_id = ? AND is_base = 0', [seasonId]);
    await executeSql(db,
      `INSERT INTO local_mod_history_entries (
        season_id, history_id, is_base, sort_order, timestamp, payload_json
      )
    ...
    await executeSql(db, 'DELETE FROM local_sync_meta WHERE season_id = ?', [seasonId]);
    await insertDerivedSeasonal(db, seasonId, null);
    await insertSyncMeta(db, seasonId, nextSyncMeta);
  });

  return { discardedCount, syncMeta: nextSyncMeta };
}
...
export async function setLocalSeasonSqlSyncMeta(
  ...
  await transaction(db, 'sync-meta', async () => {
    await executeSql(db, 'DELETE FROM local_sync_meta WHERE season_id = ?', [seasonId]);
    await insertSyncMeta(db, seasonId, syncMeta);
  });
}
