...
class DefaultTransactionDatabase extends NodeSqliteTestDatabase {
  override supportsExplicitTransactions: boolean | undefined = undefined;
  statements: string[] = [];
...
async function suppressExpectedSqlRecoveryWarning<T>(run: () => Promise<T>): Promise<T> {
  const originalWarn = console.warn;
  console.warn = (...args: unknown[]) => {
    if (String(args[0] ?? '').startsWith('[local-season-sql] recovering nested transaction')) return;
    originalWarn(...args);
  };
  try {
    return await run();
...
class ConcurrentTransactionDatabase extends NodeSqliteTestDatabase {
  inTransaction = false;
  ...
  async execute(sql: string, values: unknown[] = []): Promise<void> {
    ...
    if (/^BEGIN\b/i.test(statement)) {
      if (this.inTransaction) {
        this.nestedBeginAttempts += 1;
        return Promise.reject('error returned from database: (code: 1) cannot start a transaction within a transaction');
      }
      this.inTransaction = true;
      return super.execute(sql, values);
    }
    if (/^COMMIT\b|^ROLLBACK\b/i.test(statement)) {
      try {
    ...
    if (this.inTransaction && /^INSERT INTO local_seasons\b/i.test(statement)) {
      await delay(20);
    }
    return super.execute(sql, values);
  }
...
class NestedBeginOnceDatabase extends NodeSqliteTestDatabase {
  beginAttempts = 0;
  rollbackAttempts = 0;
  ...
  async execute(sql: string, values: unknown[] = []): Promise<void> {
    const statement = sql.trim();
    if (/^BEGIN\b/i.test(statement)) {
      this.beginAttempts += 1;
      if (this.beginAttempts === 1) {
        return Promise.reject('error returned from database: (code: 1) cannot start a transaction within a transaction');
      }
    }
    if (/^ROLLBACK\b/i.test(statement) && this.beginAttempts === 1) {
...
class NestedBeginAlwaysDatabase extends NodeSqliteTestDatabase {
  beginAttempts = 0;
  rollbackAttempts = 0;
...
test('fails closed when the SQL local store cannot be opened', async () => {
  setLocalSeasonSqlDatabaseFactoryForTests(async () => null);
  try {
    await assert.rejects(
      () => getLocalSeasonSqlDatabase(),
      /SQL local season store is unavailable/i
    );
  } finally {
    setLocalSeasonSqlDatabaseFactoryForTests(null);
  }
});
...
test('round-trips a season workspace through SQLite rows', async () => {
  ...
  assert.equal(loaded?.baseRows.length, 0);
  ...
  const sourceRows = await db.select<{ count: number }>(
    'SELECT COUNT(*) AS count FROM local_source_rows WHERE season_id = ?',
    [season.id]
  );
  assert.equal(sourceRows[0]?.count, 0);
  db.close();
});
...
test('serializes concurrent SQL workspace saves to avoid nested Tauri transactions', async () => {
  const db = new ConcurrentTransactionDatabase();
  await migrateLocalSeasonSqlDatabase(db);

  await Promise.all([
    saveLocalSeasonSqlWorkspace(db, workspace()),
    saveLocalSeasonSqlWorkspace(db, workspace()),
  ]);
  const loaded = await loadLocalSeasonSqlWorkspace(db, season.id);

  assert.equal(db.nestedBeginAttempts, 0);
  assert.equal(loaded?.records.length, 2);
  db.close();
});

test('rolls back and retries once when the SQL bridge reports a nested transaction on begin', async () => {
  const db = new NestedBeginOnceDatabase();
  await migrateLocalSeasonSqlDatabase(db);

  await suppressExpectedSqlRecoveryWarning(() => saveLocalSeasonSqlWorkspace(db, workspace()));
  const loaded = await loadLocalSeasonSqlWorkspace(db, season.id);

  assert.equal(db.beginAttempts, 2);
  assert.equal(db.rollbackAttempts, 1);
  assert.equal(loaded?.records.length, 2);
  db.close();
});

test('fails closed when nested transaction recovery cannot begin a clean transaction', async () => {
  const db = new NestedBeginAlwaysDatabase();
  await migrateLocalSeasonSqlDatabase(db);

  await suppressExpectedSqlRecoveryWarning(() =>
    assert.rejects(
      () => saveLocalSeasonSqlWorkspace(db, workspace()),
...
test('discards SQL pending changes from metadata without loading the full workspace first', async () => {
  const db = await createMigratedDb();
  await saveLocalSeasonSqlWorkspace(db, workspace());

  const summaries = await listLocalSeasonSqlPendingSummaries(db);
  ...
  const discarded = await discardLocalSeasonSqlPendingChanges(db, season.id);
  const loaded = await loadLocalSeasonSqlWorkspace(db, season.id);

  assert.equal(discarded?.discardedCount, 1);
  assert.deepEqual(loaded?.pendingOps, []);
  assert.equal(loaded?.syncMeta.pendingCount, 0);
  assert.equal(loaded?.syncMeta.lastLocalChangeAt, null);
  assert.equal(loaded?.syncMeta.syncStatus, 'synced');
  assert.equal(loaded?.records.length, 2);
  assert.equal(loaded?.records.find((entry) => entry.id === 'leg-1')?.gate, null);
  assert.equal(loaded?.modificationEntries.length, 0);
  assert.deepEqual(loaded?.modHistory, []);
  db.close();
});
