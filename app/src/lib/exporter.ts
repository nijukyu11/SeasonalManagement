...
export function validateFlightLegsForSeasonalExport(legs: FlightLeg[]): SeasonalExportValidationResult {
  ...
  const pairing = resolveSeasonalPairs(activeLegs);
  const legById = new Map(activeLegs.map((leg) => [leg.id, leg]));
  for (const pairIssue of pairing.issues) {
    const leg = legById.get(pairIssue.legId);
    if (!leg) continue;
    issues.push({
