...
### Phát hành lại đúng base `codex/web-traffic-report` — 2026-09-11

- **Port UI/contract sang worktree báo cáo** (không dùng `main`): thêm `TrafficCoverageCounts` + `kpis.coverage_counts?` + validator (`trafficReportContract.ts`), `coverageCounts?`/`coverage_counts?` + validator + decode (`trafficReportV2Contract.ts`), map adapter (`trafficReportDataAdapter.ts`), `CoverageScopeStrip` + `scrollToSection` + `jumpToNetworkSection` + anchor `#market-section`/`#airline-section` (`TrafficReportClient.tsx`). SQL không đổi (đã apply production DB).
- **Cờ build xác định từ hành vi production live** (không đoán): `NEXT_PUBLIC_TRAFFIC_WORKBOOK_EXPORT_ENABLED=true` (dialog "Xuất báo cáo" hiện), `NEXT_PUBLIC_TRAFFIC_HTML_EXPORT_ENABLED=true` (mục "Định dạng → HTML offline (.html)"), `NEXT_PUBLIC_TRAFFIC_DASHBOARD_DAILY_PUBLICATION=true` (dashboard gọi `/dashboard-publication`), `NEXT_PUBLIC_TRAFFIC_REPORT_V2_ENABLED` **không đặt** (report gọi loạt `/api/report/v1/...`), `NEXT_PUBLIC_TRAFFIC_WORKBOOK_ENABLED_REPORT_TYPES` **không đặt** (dialog hiện đủ 5 loại). Build đầu thiếu cờ export nên dialog biến mất — đã build lại kèm cờ.
- **Artifact**: `npm run build:traffic-report` với 3 cờ trên → 50 file. `reports/traffic.html` sha256 `7ece06d750af8442cbd37725c3aac8300180ccbe9e09b52c254b0cc86f7aedc6`; chunk client `0pyisclgr14k2.js` sha256 `8f91f931935f969c8ab71976a1d914cf341a1dab8d765a43c5123d5aff02c4f2`.
- **Delta so với production cũ**: mọi chunk được `traffic.html` tham chiếu **trùng hash byte-for-byte** (gồm `turbopack-0poq0sp4_jait.js`), chỉ khác duy nhất chunk client (`0~n7df-.p5b20.js` → `0pyisclgr14k2.js`) → biểu đồ/mẫu tuần-tháng/trình xuất HTML/dashboard không đổi.
- **Staging**: release `20260911T161256Z-coverage-counts-report-flagged`; Quick Tunnel `https://pretty-wing-stopping-cleaner.trycloudflare.com`. Verify browser thật: dải `253 ngày · 46 chặng bay · 45 hãng khai thác · 17 quốc gia`; pill quốc gia → `market_dimension=country`; anchor `market/airline` top 96px (reduced-motion); mobile 375px `overflow = 0`; dialog xuất khớp production; request mạng v1 khớp production; dashboard cùng read-mode Daily Publication, số liệu thật, không lỗi.
- **Người dùng accept** → publish: `current → releases/20260911T161256Z-coverage-counts-report-flagged`; `nginx -t` + reload; smoke `127.0.0.1:8780` (`/healthz`, `/reports/traffic`, `/reports/traffic.txt`, `/reports/traffic/dashboard`) = 200; chunk dải = 200; API v1 `coverage_counts` OK.
- **Public verify** `https://report.ahtops.xyz/reports/traffic` (cache tắt): dải `46 chặng bay / 45 hãng khai thác / 17 quốc gia`; overview public `coverage_counts {routes:46, airlines:45, countries:17}`; biểu đồ + "Xuất báo cáo" + dialog HTML còn nguyên; không còn nút `Tải Excel`; dashboard Daily Publication không lỗi. Origin HTML tham chiếu chunk mới, `cf-cache-status: DYNAMIC`.
