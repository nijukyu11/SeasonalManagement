...
function compileFixtureModules() {
  ...
  for (const name of ['types', 'importSeasonRules', 'iataSeason', 'parser', 'exporter', 'sourceRowPatterns', 'atomicSchedule', 'firestoreWritePlanner', 'importProgress', 'settingsRules', 'settingsPageActions', 'modHistorySizing', 'auditLog', 'detailedScheduleState', 'dailySchedule', 'dailyScheduleImport', 'dailyScheduleExport', 'checkinAllocation', 'checkInCounterSettings', 'gateAllocation', 'checkinPdfExport', 'gatePdfExport', 'seasonDataCache', 'seasonalLinkActions', 'nativeRuntime', 'nativeLocalSeasonStore', 'localSeasonStore', 'localSeasonSqlStore', 'seasonWorkspaceBootstrap', 'seasonChangeEvents', 'appSessionCleanup', 'exportSave', 'remoteStore', 'supabase', 'supabaseStore', 'seasonSync', 'seasonAutoSync', 'seasonalDisplayAggregator', 'routeCountry', 'dashboardAnalysis', 'dashboardAiShared', 'dashboardAiAnalysis', 'dashboardReportExport', 'persistenceSchema']) {
    ...
    fs.writeFileSync(path.join(tempDir, `${name}.js`), output.outputText);
  }
}
...
async function run() {
  ...
  const XLSX = require('xlsx');
  const { getDirtyImportGuard, normalizeSeasonSheetName } = require(path.join(tempDir, 'importSeasonRules.js'));
  ...
  const inferredJuneMayRequest = inferDashboardAiDataRequestFromText({
    userPrompt: 'tạo bảng so sánh các điểm khác biệt nổi bật của tháng 6 với tháng 5 theo hãng bay và đường bay',
    assistantText: 'Payload hiện tại chưa có bảng phân rã chi tiết theo hãng bay hay đường bay. Hệ thống sẽ gửi yêu cầu truy xuất dữ liệu so sánh chi tiết giai đoạn này.',
    context: aiContext,
  });
  ...
  assert(
    isDashboardAiDataRequestPrompt('Payload hiện tại chưa có bảng phân rã chi tiết theo hãng bay hay đường bay. Hệ thống sẽ gửi yêu cầu truy xuất dữ liệu so sánh chi tiết giai đoạn này.') &&
      ...
      deterministicJuneMayAnswer.boardPatch?.blocks.some((block) => block.table?.templateId === 'comparison-drivers' && block.table?.filters?.currentMonth === '06' && block.table?.filters?.previousMonth === '05') &&
      deterministicJuneMayAnswer.boardPatch?.blocks.some((block) => block.table?.filters?.dimension === 'route'),
  ...
  assert(
    ...
    `AI settings should default to Gemini with optional Qwen-compatible model metadata, got ${JSON.stringify(DEFAULT_AI_ANALYSIS_SETTINGS)}`
  );
  ...
  assert(
    DASHBOARD_AI_TOOL_REGISTRY.every((tool) => tool.toolset && Array.isArray(tool.requires) && tool.outputContract) &&
      ...
      toolsetAvailabilityReady.find((tool) => tool.name === 'suggest_template_report')?.toolset === 'dashboard-export',
  ...
  const monthDriverSkill = resolveDashboardAiSkillForPrompt('tạo bảng so sánh các điểm khác biệt nổi bật của tháng 6 với tháng 5 theo hãng bay và đường bay');
  const peakHourSkill = resolveDashboardAiSkillForPrompt('vẽ biểu đồ peak hour cho mùa đang chọn');
  assert(
    DASHBOARD_AI_SKILL_REGISTRY.every((skill) => skill.id && skill.descriptionVi && Array.isArray(skill.triggersVi) && skill.preferredTool) &&
      ...
      monthDriverSkill.contextProfile === 'comparison-drivers' &&
      monthDriverSkill.blocks.some((block) => block.table?.templateId === 'comparison-drivers') &&
      peakHourSkill?.id === 'peak-hour-analysis' &&
      peakHourSkill.contextProfile === 'peak-hour',
    `Dashboard AI skill registry should route Vietnamese prompts to report skills, got ${JSON.stringify({ skills: DASHBOARD_AI_SKILL_REGISTRY, monthDriverSkill, peakHourSkill })}`
  );
  const notebookContext = buildDashboardAiNotebookContext([
    {
      id: 'cell-1',
      prompt: 'tạo bảng so sánh tháng 6 với tháng 5',
      assistantText: 'Đã tạo bảng so sánh theo hãng bay.',
      blocks: deterministicJuneMayAnswer.boardPatch?.blocks ?? [],
      toolTraceSummary: [{ tool: 'compose_dashboard_ai_board', status: 'accepted', reason: 'Đã tạo block so sánh.' }],
      exportAction: null,
      createdAt: 1,
      modelId: 'qwen-plus',
    },
  ...
  assert(
    notebookContext.cells.length === 1 &&
      notebookContext.cells[0].blockSummaries.some((block) => block.type === 'table' && block.templateId === 'comparison-drivers') &&
      notebookContext.cells[0].toolTraceSummary[0].reason === 'Đã tạo block so sánh.' &&
  ...
  const vietnamesePrompt = buildDashboardAiPrompt({
    userPrompt: 'Tạo bảng so sánh tháng 6 với tháng 5',
    context: aiContext,
  ...
  assert(
    vietnamesePrompt.includes('STABLE_AGENT_CONTRACT') &&
      ...
      (vietnamesePrompt.includes('Luôn trả lời bằng tiếng Việt') || vietnamesePrompt.includes('LuÃ´n tráº£ lá»i báº±ng tiáº¿ng Viá»‡t')) &&
  ...
  assert(
    functionRequestWithHermesPattern.language === 'vi' &&
      ...
      functionRequestWithHermesPattern.contextProfile === 'comparison-drivers' &&
  ...
  assert(
    JSON.stringify(DASHBOARD_AI_TOOL_REGISTRY.map((tool) => tool.name)) === JSON.stringify(['query_dashboard_data', 'request_dashboard_data_slice', 'suggest_template_report', 'suggest_custom_workbook', 'suggest_visual_report', 'compose_dashboard_ai_board']) &&
      ...
      resolveDashboardAgentToolForPrompt('xuất sản lượng tháng 7 sang Excel') === 'suggest_template_report' &&
      resolveDashboardAgentToolForPrompt('vẽ peak hour visual report cho tháng này') === 'suggest_visual_report' &&
      resolveDashboardAgentToolForPrompt('so sánh tháng 8 VJ ngoài bảng hiện tại') === 'query_dashboard_data' &&
      resolveDashboardAgentToolForPrompt('tạo workbook riêng gồm airline và route') === 'suggest_custom_workbook',
  ...
  assert(
    resolveDashboardAiWorkspaceToolForPrompt('Build visual report') === 'compose_dashboard_ai_board' &&
      resolveDashboardAiWorkspaceToolForPrompt('Draw peak hour chart') === 'compose_dashboard_ai_board' &&
      resolveDashboardAiWorkspaceToolForPrompt('Create driver table') === 'compose_dashboard_ai_board' &&
      resolveDashboardAiWorkspaceToolForPrompt('Compare selected seasons') === 'compose_dashboard_ai_board' &&
      resolveDashboardAiWorkspaceToolForPrompt('tạo bảng trắng so sánh 3 mùa') === 'compose_dashboard_ai_board' &&
      resolveDashboardAiWorkspaceToolForPrompt('vẽ biểu đồ peak hour') === 'compose_dashboard_ai_board' &&
      resolveDashboardAiWorkspaceToolForPrompt('lập report dạng bảng') === 'compose_dashboard_ai_board',
    'AI Workspace visual/table/chart presets and Vietnamese prompts must route to compose_dashboard_ai_board'
  );
  ...
  const safeBoardPatch = resolveDashboardAiBoardPatch({
    ...
    blocks: [
      ...
      },
      {
        ...
        title: 'Driver Table',
        source: 'comparison',
        table: { templateId: 'comparison-drivers', title: 'Drivers', columns: ['label', 'current', 'previous', 'delta', 'deltaPct'], source: 'comparison', limit: 40 },
      },
      {
        id: 'notes',
        type: 'insight-list',
  ...
  const safeQueryResults = resolveDashboardAiQueryResults([{
    ...
    rowCount: 2,
    truncated: false,
    dataQualityNotes: ['Nguồn dữ liệu reporting.flight_operations'],
  }]);
  const queryBoardPatch = buildDashboardAiBoardPatchFromQueryResults(safeQueryResults, 'Top 10 route có PAX cao nhất trong tháng 3');
  assert(
  ...
  const dateRangeQuery = inferDashboardAiDataQueryForPrompt({
    userPrompt: 'thống kê từ 01/06/2026 đến 07/06/2026',
    context: aiContext,
  });
  const consecutiveSeasonQuery = inferDashboardAiDataQueryForPrompt({
    userPrompt: 'so sánh mùa liên tiếp',
    context: {
      ...aiContext,
      multiSeasonCatalog: {
  ...
  const countryQuery = inferDashboardAiDataQueryForPrompt({
    userPrompt: 'top quốc gia có nhiều chuyến bay nhất',
    context: aiContext,
  });
  const aircraftQuery = inferDashboardAiDataQueryForPrompt({
    userPrompt: 'thống kê theo máy bay tháng 7',
    context: aiContext,
  });
  const arrDepMixQuery = inferDashboardAiDataQueryForPrompt({
    userPrompt: 'tổng ARR/DEP mix tháng 7',
    context: aiContext,
  });
  assert(
  ...
  assert(
    richNotebookPatch?.blocks.some((block) => block.type === 'rich-markdown' && block.markdown?.content.includes('| Chỉ tiêu | Giá trị |')) &&
  ...
  const peakHourFallbackPatch = buildDashboardAiFallbackBoardPatch({
    userPrompt: 'vẽ biểu đồ peak hour',
    preferredTool: 'compose_dashboard_ai_board',
  });
  const tableFallbackPatch = buildDashboardAiFallbackBoardPatch({
    userPrompt: 'lập report dạng bảng',
    preferredTool: 'compose_dashboard_ai_board',
  });
  assert(
  ...
  assert(
    comparisonDifferenceFallbackPatch?.blocks[0]?.table?.templateId === 'comparison-drivers' &&
      ...
      comparisonDifferenceFallbackPatch.blocks.some((block) => block.insights?.some((insight) => insight.includes('dashboard'))) &&
    ...
    `AI Workspace fallback should map month-over-month difference table prompts to comparison driver blocks, got ${JSON.stringify(comparisonDifferenceFallbackPatch)}`
  );
  ...
  const staleProviderBoardAnswer = await analyzeDashboardWithAi({
    ...
    supabaseClient: {
      functions: {
        invoke: async () => ({
          data: {
            assistantText: 'Đã truy vấn khoảng ngày.',
            queryResults: [{
              queryId: 'date-range',
              view: 'flight_operations',
              ...
              rowCount: 1,
              truncated: false,
              dataQualityNotes: ['Nguồn dữ liệu: reporting.flight_operations; queryId=date-range.'],
            }],
            boardPatch: safeBoardPatch,
          },
          error: null,
  ...
  const transientRetryAnswer = await analyzeDashboardWithAi({
    userPrompt: 'Tạo báo cáo trực quan',
    context: aiContext,
    model: resolvedAiModel,
    preferredTool: 'compose_dashboard_ai_board',
    providerFallback: true,
    supabaseClient: {
      functions: {
        invoke: async () => {
          ...
          return {
            data: {
              assistantText: 'Đã tạo báo cáo trực quan từ dữ liệu dashboard.',
  ...
  assert(
    retryAttempts === 2 &&
      (transientRetryAnswer.assistantText.includes('Đã tạo') || transientRetryAnswer.assistantText.includes('ÄÃ£ táº¡o')) &&
      transientRetryAnswer.toolTraceSummary.some((trace) => trace.providerAttempt === 2 && (trace.reason.includes('Thử lại') || trace.reason.includes('Thá»­ láº¡i'))),
  ...
  try {
    await analyzeDashboardWithAi({
      userPrompt: 'Tạo báo cáo lỗi schema',
      context: aiContext,
      model: resolvedAiModel,
      providerFallback: true,
      supabaseClient: {
  ...
  const localDeltaCommitEnd = localSeasonStoreSource.indexOf('export async function applyLocalModificationBatch(', localDeltaCommitStart + 1);
  const localDeltaCommitBody = localDeltaCommitStart >= 0 && localDeltaCommitEnd > localDeltaCommitStart
    ...
    : '';
  assert(
    typeof applyLocalModificationBatchDelta === 'function' &&
      checkInWorkerSource.includes('applyLocalModificationBatchDelta') &&
      !checkInWorkerCommitBody.includes('loadLocalSeasonWorkspace') &&
      !checkInWorkerCommitBody.includes('applyLocalModificationBatch(') &&
      localSeasonStoreSource.includes('function pickHydratedModificationsById') &&
      localDeltaCommitBody.includes('pickHydratedModificationsById(data.modificationEntries, affectedLegIds)') &&
      localDeltaCommitBody.includes('pickHydratedModificationsById(data.baseModificationEntries ?? data.modificationEntries, affectedLegIds)') &&
      !localDeltaCommitBody.includes('deserializeModificationEntries(data.modificationEntries)') &&
      !localDeltaCommitBody.includes('deserializeModificationEntries(data.baseModificationEntries ?? data.modificationEntries)') &&
      localDeltaCommitBody.includes("tx.objectStore('pendingOps').put") &&
      localDeltaCommitBody.includes("tx.objectStore('syncMeta').put") &&
      localDeltaCommitBody.includes("tx.objectStore('derivedSeasonal').put") &&
      !localDeltaCommitBody.includes("tx.objectStore('seasonData').put"),
    'Check-in local commit worker must use a delta-only IndexedDB modification commit instead of hydrating and rewriting the full season workspace after the debounce'
  );
  assert(
  ...
  const pendingSummaryBody = pendingSummaryStart >= 0 && pendingSummaryEnd > pendingSummaryStart
    ...
    : '';
  assert(
    pendingSummaryBody.includes("db.get('syncMeta', seasonId)") &&
      pendingSummaryBody.includes('canUseIndexedDb()') &&
      pendingSummaryBody.includes('loadLocalSeasonWorkspace') &&
      pendingSummaryBody.indexOf("db.get('syncMeta', seasonId)") < pendingSummaryBody.indexOf('loadLocalSeasonWorkspace'),
    'Check-in local workspace change notifications must read pending sync summary from syncMeta before falling back to full workspace hydration'
  );
  assert(
  ...
  const gateSettingsTabButtonSource = settingsPageSource.slice(
    settingsPageSource.indexOf("onClick={() => setActiveTab('gateAllocation')}"),
    settingsPageSource.indexOf("onClick={() => setActiveTab('airlineColors')}")
  );
  const gateSettingsSectionSource = settingsPageSource.slice(
  ...
  assert(
    settingsPageSource.includes("type SettingsTab = 'groups' | 'rules' | 'checkinCounters' | 'gateAllocation' | 'routeCountries' | 'airlineColors'") &&
    settingsPageSource.includes("setActiveTab('airlineColors')") &&
      settingsPageSource.includes('Airline Colors') &&
      settingsPageSource.includes('type=\"color\"') &&
      settingsPageSource.includes('airlineColors') &&
      settingsPageSource.includes('addAirlineColor') &&
      settingsPageSource.includes('updateAirlineColor') &&
      settingsPageSource.includes('deleteAirlineColor'),
  ...
  assert(
    settingsPageSource.includes("'routeCountries'") &&
      settingsPageSource.includes("setActiveTab('routeCountries')") &&
      settingsPageSource.includes('Route-Country') &&
      settingsPageSource.includes('routeCountrySearch') &&
      settingsPageSource.includes('addOrUpdateRouteCountry') &&
      settingsPageSource.includes('updateRouteCountry') &&
      settingsPageSource.includes('deleteRouteCountry') &&
      settingsPageSource.includes('handleRouteCountryImport') &&
      settingsPageSource.includes('accept=".xls,.xlsx"') &&
      settingsPageSource.includes('parseRouteCountryRows') &&
      settingsPageSource.includes('mergeRouteCountryMappings') &&
      settingsPageSource.includes('Update From Excel') &&
      settingsPageSource.includes('Update Route-Country Map') &&
      !settingsPageSource.includes('Replace From Excel') &&
      !settingsPageSource.includes('Replace Route-Country Map') &&
      settingsPageSource.includes('routeCountries'),
  ...
  assert(
    settingsPageSource.includes("setActiveTab('gateAllocation')") &&
      settingsPageSource.includes("activeTab === 'gateAllocation' ? 'border-b-2 border-primary text-primary'") &&
      ...
      gateSettingsSectionSource.includes('Gate Lock') &&
      gateSettingsSectionSource.includes('Add Gate Lock') &&
      gateSettingsSectionSource.includes('addGateLock') &&
      gateSettingsSectionSource.includes('toggleGateLockGate') &&
      gateSettingsSectionSource.includes('gateLocks'),
    'Settings Gate tab must be titled Gate and expose add-gate, add-group, and gate-lock controls'
  );
  ...
  assert(
    gatePageSource.includes('Gate Allocation') &&
      ...
      settingsPageSource.includes("setActiveTab('gateAllocation')") &&
      settingsPageSource.includes('Gate Inventory') &&
      settingsPageSource.includes('Stand Mapping Table') &&
      settingsPageSource.includes('toggleGateGroupGate') &&
  ...
  assert(
    gateWorkerSource.includes('applyLocalModificationBatchDelta') &&
      ...
      gateFlushBody.includes("publishWorkspaceChange(season.id, result.syncMeta.localRevision, result.source ?? 'gate-worker')") &&
  ...
  assert(
    detailedPageSource.includes('applyLocalModificationBatchDelta') &&
      ...
      detailedPageSource.includes('await applyLocalModificationBatchDelta(season.id, regularMods, {') &&
      detailedPageSource.indexOf('setCurrentMods(nextMods);') < detailedPageSource.indexOf('await applyLocalModificationBatchDelta(season.id, regularMods, {') &&
      !detailedPageSource.includes('const workspace = await applyLocalModificationBatch(season.id, regularMods, historyEntry);'),
    'Detailed Schedule regular mass edits/deletes must optimistically update visible state, persist only selected IDs, and rollback immediately on failed delta commits'
  );
  ...
  assert(
    seasonalPageSource.includes("import { filterUiUndoEntriesForSession, trimUiUndoEntries } from '@/lib/uiUndoMemory';") &&
      ...
      seasonalPageSource.includes('setModHistory(trimUiUndoEntries(filterUiUndoEntriesForSession(workspace.modHistory)));') &&
      detailedPageSource.includes('setModHistory(trimUiUndoEntries(filterUiUndoEntriesForSession(workspace.modHistory)));') &&
      seasonalPageSource.includes('const historyToUndoFrom = workspace.modHistory;') &&
      detailedPageSource.includes('const historyToUndoFrom = workspace.modHistory;') &&
  ...
  assert(
    seasonDataCacheSource.includes('subscribeSeasonWorkspaceChanges') &&
      ...
      seasonWorkspaceRefreshHookSource.includes('startTransition') &&
    ...
    'Cached flight-data modules must subscribe to same-session season workspace changes and refresh inactive route panels only on activation'
  );
  assert(
    [seasonalPageSource, detailedPageSource, dailyPageSource, checkInPageSource, gatePageSource].every((source) =>
      ...
      checkInWorkerSource.includes('localRevision') &&
      gateWorkerSource.includes('localRevision') &&
  ...
  assert(
    seasonSyncProviderSource.includes('const WORKSPACE_CHANGE_DEBOUNCE_MS = 200') &&
      ...
      seasonSyncProviderSource.includes('void getPendingSyncSummary(seasonId).then((summary) =>') &&
      seasonSyncProviderSource.includes('source,'),
    'SeasonSyncProvider must debounce rapid workspace-change notifications into one pending-summary read and scheduler notification per season'
  );
  ...
  assert(
    localSeasonStoreSource.includes("import { deleteDB, openDB") &&
      localSeasonStoreSource.includes('function closeLocalSeasonDb') &&
      localSeasonStoreSource.includes('db.close()') &&
      localSeasonStoreSource.includes('dbPromise = null') &&
      localSeasonStoreSource.includes('export async function clearAllLocalSeasonWorkspaces') &&
      localSeasonStoreSource.includes('await deleteDB(DB_NAME') &&
      localSeasonStoreSource.includes('memoryWorkspaces.clear()') &&
  ...
  assert(
    appShellSource.includes("import NativeStartupSessionReset from './NativeStartupSessionReset';") &&
      ...
      nativeStartupSessionResetSource.includes('discardPendingLocalChanges: true') &&
      nativeStartupSessionResetSource.includes('resetUndoSession: true') &&
    ...
    'Native startup must discard pending local edits and Undo history while preserving downloaded season baselines'
  );
  assert(
  ...
  assert(
    seasonAutoSyncSource.includes("status: 'dirty'") &&
      seasonAutoSyncSource.includes("message: summary.pendingCount === 0 ? null : 'Unsynced local changes. Use Sync now to push them to the server.'") &&
  ...
  assert(
    seasonSyncSource.includes('export const SNAPSHOT_CATCHUP_BACKLOG_THRESHOLD = 100') &&
      ...
      seasonSyncProviderSource.includes('Updating in background: 0 / ${backlog}') &&
      ...
      seasonSyncProviderSource.includes("reason: 'Manual sync running'") &&
      [seasonalPageSource, detailedPageSource, dailyPageSource, checkInPageSource, gatePageSource].every((source) =>
  ...
  assert(
    supabaseStoreSource.includes('const SUPABASE_SELECT_PAGE_SIZE = 1000') &&
      ...
      supabaseStoreSource.includes("selectAllRows<SourceRowRelationalRow>('season_source_rows'") &&
      supabaseStoreSource.includes("selectAllRows<SourceRowDayRelationalRow>('season_source_row_days'") &&
      ...
      [seasonalPageSource, detailedPageSource, dailyPageSource, dashboardPageSource, checkInPageSource, gatePageSource].every((source) =>
        source.includes('loadOrSeedSeasonWorkspace')
      ),
    'Supabase large season reads must be paginated, imports must verify remote counts, and stale dataVersion workspaces must reload full baseline instead of showing capped local data'
  );
  assert(
      ...
      seasonSyncSource.includes('mergeSeasonSnapshotIntoLocalWorkspace') &&
      ...
      seasonSyncProviderSource.includes('server-bootstrap') &&
      seasonSyncProviderSource.includes('runNativeSeasonCatchup') &&
      seasonSyncProviderSource.includes("'native-catchup'") &&
      ...
      seasonConflictResolutionSource.includes('resolveLocalSeasonConflict') &&
  ...
  assert(
    localSeasonStoreSource.includes('nativeFullSaveReason') &&
      ...
      seasonalPageSource.includes("nativeFullSaveReason: 'import-reset'") &&
  ...
  assert(
    ...
    'All module sync buttons must use the shared immediate-lock SyncActionButton with spinner, aria-busy, and stable width'
  );
  assert(
  ...
  assert(
    ...
    'Supabase v2 sync must scope entity clocks by season, make duplicate op retries idempotent, store field versions as applied server_seq, and return applied events plus server high-water for exact catch-up'
  );
  assert(
  ...
  assert(
    remoteStoreSource.includes('RemoteDashboardSeasonData') &&
      ...
      seasonWorkspaceBootstrapSource.includes('loadDefaultServerBaseline') &&
      seasonWorkspaceBootstrapSource.includes('getSourceRows') &&
    ...
    'Dashboard and AI data loading must use the shared local-first workspace bootstrap instead of page-local payload/season-only reads'
  );
  ...
  assert(
    tauriConfigSource.includes('"frontendDist": "../out"') &&
      tauriConfigSource.includes('"beforeBuildCommand": "npm run build"') &&
  ...
  assert(
    dashboardPageSource.includes('Seasonal Dashboard') &&
      !dashboardPageSource.includes('Bảng điều khiển hiệu suất mùa bay') &&
      ...
      !dashboardPageSource.includes('Ä') &&
  ...
  const rotateAiFunctionSource = fs.readFileSync(path.join(root, 'supabase', 'functions', 'rotate-dashboard-ai-key', 'index.ts'), 'utf8');
  ...
  assert(
    dashboardAiWorkspaceUiSource.includes('AI Workspace') &&
      dashboardAiWorkspaceUiSource.includes('data-testid="ai-notebook-canvas"') &&
      dashboardAiWorkspaceUiSource.includes('data-testid="ai-notebook-cell"') &&
      ...
      aiNotebookBlockRenderersSource.includes('sandbox="allow-popups-to-escape-sandbox"') &&
      ...
      dashboardPageSource.includes('Đang truy xuất dữ liệu dashboard local...') &&
      dashboardPageSource.includes('buildDashboardAiContext') &&
      dashboardPageSource.includes('buildDashboardAiResolvedDataRequest') &&
      dashboardPageSource.includes('inferDashboardAiDataRequestFromText') &&
      dashboardPageSource.includes('buildDashboardAiResolvedDataFallbackAnswer') &&
      dashboardPageSource.includes('isDashboardAiDataRequestPrompt') &&
  ...
  assert(
    dashboardAiSource.includes('DASHBOARD_CONTEXT_JSON') &&
      ...
      (dashboardAiSource.includes('Luôn trả lời bằng tiếng Việt') || dashboardAiSource.includes('LuÃ´n tráº£ lá»i báº±ng tiáº¿ng Viá»‡t')) &&
  ...
  assert(
    dashboardReportExportSource.includes('CANONICAL_DASHBOARD_REPORT_COLUMNS') &&
      ...
      aiWorkspacePanelSource.includes('Xuất Excel MoM/WoW') &&
    ...
    'Dashboard AI export harness must expose fixed workbook templates, generated summary sheets, and inline notebook download controls'
  );
  assert(
    settingsPageSource.includes('AI Analysis') &&
      settingsPageSource.includes('rotateDashboardAiProviderKey') &&
      settingsPageSource.includes('can_manage_ai') &&
      settingsPageSource.includes('Add AI Model') &&
      settingsPageSource.includes('Rotate Provider Key') &&
      settingsPageSource.includes('aiAnalysis'),
    'Settings route must expose AI Analysis model configuration and admin-only provider key rotation controls'
  );
  assert(
    ...
    'Supabase schema must add app_operators.can_manage_ai bootstrap migration and must not add remote AI chat history tables'
  );
  assert(
    dashboardAiFunctionSource.includes('dashboard-ai-analysis') &&
      ...
      (dashboardAiFunctionSource.includes('Luôn trả lời bằng tiếng Việt') || dashboardAiFunctionSource.includes('LuÃ´n tráº£ lá»i báº±ng tiáº¿ng Viá»‡t')) &&
      ...
      dashboardAiFunctionSource.includes('Read-only local dashboard data requests do not require user confirmation') &&
      dashboardAiFunctionSource.includes('do not ask for data again') &&
    ...
    'dashboard-ai-analysis Edge Function must validate operators, inject grounding context, support data/export action payloads, and call provider secrets server-side'
  );
  assert(
      rotateAiFunctionSource.includes('rotate-dashboard-ai-key') &&
      rotateAiFunctionSource.includes('can_manage_ai') &&
      rotateAiFunctionSource.includes(".eq('user_id', userData.user.id)") &&
      dashboardAiAdminSource.includes('auth.getUser()') &&
      dashboardAiAdminSource.includes(".eq('user_id', userData.user.id)") &&
      rotateAiFunctionSource.includes('/v1/projects/${projectRef}/secrets') &&
      rotateAiFunctionSource.includes('DASHBOARD_AI_MANAGEMENT_ACCESS_TOKEN') &&
      rotateAiFunctionSource.includes('DASHBOARD_AI_PROJECT_REF') &&
      !rotateAiFunctionSource.includes('return Response.json({ apiKey') &&
      !rotateAiFunctionSource.includes('assistantText'),
    'AI key rotation Edge Function must require can_manage_ai and update Supabase secrets without returning plaintext keys'
  );
  assert(
  ...
  assert(
    settingsPageSource.includes('A/C Group') &&
      settingsPageSource.includes('Default Counter Allocation') &&
      ...
      settingsPageSource.includes('datetime-local') &&
      settingsPageSource.includes('BHS') &&
      settingsPageSource.includes('Transit') &&
      settingsPageSource.includes('Delete Counter') &&
      settingsPageSource.includes('Delete Counter Group') &&
      settingsPageSource.includes('Delete Counter Lock') &&
      settingsPageSource.includes('No A/C groups yet') &&
      settingsPageSource.includes('No counter rules yet'),
  ...
  assert(
    settingsPageSource.includes('confirmCheckInCounterLabelChange') &&
      ...
      settingsPageSource.includes('onBlur={(event) => void confirmCheckInCounterLabelChange(counter, event.target.value)}') &&
      settingsPageSource.includes('toggleCheckInCounterGroupCounter(group.id, counter.id)') &&
      settingsPageSource.includes('toggleCheckInCounterLockCounter(lock, counter.id)'),
    'Settings page must confirm identity-changing counter renames before calling the pure rename helper'
  );
  assert(
    seasonalPageSource.includes("router.push('/settings')") &&
      dailyPageSource.includes("router.push('/settings')") &&
      detailedPageSource.includes("router.push('/settings')"),
    'Settings gear navigation must be wired from Seasonal, Daily, and Detailed sidebars'
  );
  assert(
